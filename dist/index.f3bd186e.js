    ...
    data.forEach(function(movie) {
      ...
      ----------------------- */ // Create favourites button HTML Element
        ...
        button.addEventListener("click", function(event) {
            ...
            for(var i = 0; i < localStorage.length; i++)// compare each value for every key in local storage with input value from user
            if (localStorage.getItem(localStorage.key(i)) == value) // set exists to true only if value found
            exists = true;
            // check exists boolean and add item if value doesn't exist
            if (exists == false) localStorage.setItem(key, value);
